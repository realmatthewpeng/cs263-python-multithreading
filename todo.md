# Potential TODO Work

- [x] Gradually increase the number of threads for a multithreaded program (fibonacci)
- [x] Test memory safety (access a shared counter and measure error rate)
- [ ] Compare different locking mechanisms to support memory safety
- [x] Compare multithreading between GIL and non-GIL versions
- [ ] Compare performance with multiprocessing instead
- [x] CPU-bound programs and I/O-bound programs
- [ ] Run some tests from <https://openbenchmarking.org/tests>
  - [x] <https://pyperformance.readthedocs.io/index.html>
  - [ ] <https://svn.python.org/projects/python/trunk/Tools/pybench/>
- [ ] Compare 3.13t with GIL enabled and disabled
- [x] Measure overhead cost of creating many threads
  - [x] Compared to overhead cost of creating processes
- [ ] Write final report: [Google Docs Link](https://docs.google.com/document/d/1Eopt7RceXUo7Wm5FUaPYfvA-DGV4bef0EngoU9AJl7I/edit?usp=sharing)
